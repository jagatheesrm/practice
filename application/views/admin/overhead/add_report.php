...
<div class="modal fade" id="add-items" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('overhead/create_report'),array('id'=>'data-id')); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">
                <span class="edit-title"><?php echo 'Edit Overhead Report'; ?></span>
                    <span class="add-title"><?php echo 'New Overhead Report'; ?></span>
                </h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php $value = (isset($report) ? $report: ''); ?>
                        <?php $attrs = (isset($report) ? array() : array('autofocus' => true, 'placeholder' => 'Enter  Cycle')); ?>
                        <?php echo render_input('report', 'report', $value, 'text', $attrs); ?>
...
<script>
    window.addEventListener('load', function() {
        ...
        if (typeof(group_id) !== 'undefined') {
            ...
            $('#add-items input[name="report"]').val($(invoker).parents('tr').find('td').eq(1).find('b').text());
